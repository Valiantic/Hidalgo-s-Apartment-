...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $residents = htmlspecialchars(trim($_POST['residents']));
    ...
    try {
        ...
        $tenant_query = "UPDATE tenant 
                         SET fullname = ?, phone_number = ?, work = ?, downpayment = ?, advance = ?, electricity = ?, water = ?, units = ?, residents = ?
                         WHERE tenant_id = ?";
        $tenant_stmt = $conn->prepare($tenant_query);
        $tenant_stmt->bind_param("sssssssssi", $fullname, $phone_number, $work, $downpayment, $advance, $electricity, $water, $unit, $residents, $tenant_id);
        $tenant_stmt->execute();
